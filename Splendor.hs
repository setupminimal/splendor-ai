...
updateState (TakeTwo gem) State{..} = (if player == players then Max else Min, State (player `mod` players + 1) hands' bank' onTable remaining)
  where
    ...
    tt Hand{..} = Hand (coins `plus` gemBag) cards score
...
updateState (BuyCard card) State{..} = (Chaos, State (-(player `mod` players + 1)) hands' bank' onTable' remaining)
  where
    ...
    addCard Hand{..} = Hand (coins `discount` payment) (addToBag (gem (fromNum card)) cards) (score + (100 * points (fromNum card)) + 1)
...
children State{..} = buyCards ++ twoCoins ++ threeCoins
  where
    buyCards = do
    ...
    twoCoins = (if diamond bank >= 4 then [TakeTwo Diamond] else [])
               ++ (if saphire bank >= 4 then [TakeTwo Saphire] else [])
               ++ (if emerald bank >= 4 then [TakeTwo Emerald] else [])
               ++ (if ruby bank >= 4 then [TakeTwo Ruby] else [])
               ++ (if onyx bank >= 4 then [TakeTwo Onyx] else [])
